...
regitPublic.controller("SignUpBusinessAccountController",
    ['$scope', '$rootScope', '$http', 'BusinessAccountService', 'AuthorizationService', 'SweetAlert', '$timeout',
        function ($scope, $rootScope, $http, _baService, _authService, _sweetAlert, $timeout) {
            ...
            $scope.register = function () {
                ...
                $scope.account.Email = $scope.account.Email.toLowerCase();
...
/*                    _authService.VerifyPhoneNumber({PhoneNumber: model.PhoneNumberCountryCallingCode + model.PhoneNumber})
            ...
            $scope.checkPinSMS = function (value) {
                ...
                _authService.CheckSetPIN($scope.authentication).then(function (response) {
                    if (!response) $scope.errors.PIN = 'Incorrect PIN. Please re-enter';
                    else
            ...
            function validateStep3() {
                ...
                if (!$scope.companyDetails.Country) {
                    valid = false;
                    $scope.progress.Errors.Step3.Country = $rootScope.translate('Country_Error_Required');
                }
