...
//5.2 Driver License
myApp.getController('DriverLicenseController',
...
function ($scope, $rootScope, $http, _userManager, _sweetAlert, _authService, alertService, VaultInformationService, CountryCityService, dateFilter, DocumentVaultService) {
    ...
    "use strict";
    var BasicInfo = VaultInformationService.VaultInformation.groupGovernmentID.value.driverLicenseCard;
    ...
    $scope.InitData = function () {
        ...
        $(BasicInfo.value).each(function (index, object) {
            ...
            var _jspath = "/Government ID/Driver License Card/" + BasicInfo.value[index]._id;
            var today = new Date();
            var expiryNo = false;
        ...
        maxindex = maxindex + 1;
        $rootScope._newJspath = "/Government ID/Driver License Card/" + maxindex;
        $scope._new =
           {
               '_id': maxindex,
               'IsEdit': false,
        ...
        $scope.addNewForm = function (newForm) {
            var jpd = "/Government ID/Driver License Card/" + newForm._id;
        ...
        $scope.removeFormDoc = function (_value) {
            ...
            var _jsp = "/Government ID/Driver License Card/" + _value._id;
    ...
    $scope.$on('driverLicenseCard', function () {
        $scope.InitData();
    });
    $scope.InitData();
    $scope.Edit = function () {
    ...
    $scope.Save = function () {
        ...
        //Save
        VaultInformationService.VaultInformation.groupGovernmentID.value.driverLicenseCard = BasicInfo;
        VaultInformationService.VaultInformation.groupGovernmentID.value.driverLicenseCard.value = lstFormSave;
        ...
        VaultInformationService.SaveVaultInformation(VaultInformationService.VaultInformation,
            VaultInformationService.VaultInformation._id).then(function (response) {
                $scope.IsEdit = false;
                $scope._new = "";
                $rootScope.$broadcast('driverLicenseCard');
