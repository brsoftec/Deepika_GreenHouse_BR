...
//5.3 Health Card
myApp.getController('HealthCardController',
...
function ($scope, $rootScope, $http, _userManager, _sweetAlert, _authService, alertService, VaultInformationService, CountryCityService, dateFilter, DocumentVaultService) {
    ...
    $scope.InitData = function () {
        ...
        $(BasicInfo.value).each(function (index, object) {
            ...
            var _jspath = "/Government ID/Health Card/" + BasicInfo.value[index]._id;
            var today = new Date();
            var expiryNo = false;
        ...
        maxindex = maxindex + 1;
        $rootScope._newJspath = "/Government ID/Health Card/" + maxindex;
        $scope._new =
           {
               '_id': maxindex,
               'IsEdit': false,
        ...
        $scope.addNewForm = function (newForm) {
            var jpd = "/Government ID/Health Card/" + newForm._id;
        ...
        $scope.removeFormDoc = function (_value) {
            ...
            var _jsp = "/Government ID/Health Card/" + _value._id;
    ...
    $scope.Save = function () {
        ...
        //Save
        VaultInformationService.VaultInformation.groupGovernmentID.value.healthCard = BasicInfo;
        VaultInformationService.VaultInformation.groupGovernmentID.value.healthCard.value = lstFormSave;
        ...
        VaultInformationService.SaveVaultInformation(VaultInformationService.VaultInformation,
            VaultInformationService.VaultInformation._id).then(function (response) {
                $scope.IsEdit = false;
                $scope._new = "";
                $rootScope.$broadcast('healthCard');
