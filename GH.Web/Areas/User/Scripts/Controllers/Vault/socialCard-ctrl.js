...
//5.5
myApp.getController('SocialCardController',
...
function ($scope, $rootScope, $http, _userManager, _sweetAlert, _authService, alertService, VaultInformationService, CountryCityService, dateFilter, DocumentVaultService) {
    ...
    "use strict";
    var BasicInfo = VaultInformationService.VaultInformation.groupGovernmentID.value.socialInsuranceCard;
    ...
    $scope.InitData = function () {
        ...
        $(BasicInfo.value).each(function (index, object) {
            ...
            var _jspath = "/Government ID/Social Insurance Card/" + BasicInfo.value[index]._id;
            var today = new Date();
            var expiryNo = false;
              // indefinite
        ...
        maxindex = maxindex + 1;
        $rootScope._newJspath = "/Government ID/Social Insurance Card/" + maxindex;
        $scope._new =
           {
               '_id': maxindex,
               'IsEdit': false,
  ...
  var jpd = "/Government ID/Social Insurance Card/" + newForm._id;
        ...
        $scope.removeFormDoc = function (_value) {
            ...
            var _jsp = "/Government ID/Social Insurance Card/" + _value._id;
    ...
    $scope.$on('socialInsuranceCard', function () {
        $scope.InitData();
    });
    $scope.InitData();
    $scope.Edit = function () {
    ...
    $scope.Save = function () {
        ...
        //Save
        VaultInformationService.VaultInformation.groupGovernmentID.value.socialInsuranceCard = BasicInfo;
        VaultInformationService.VaultInformation.groupGovernmentID.value.socialInsuranceCard.value = lstFormSave;
        ...
        VaultInformationService.SaveVaultInformation(VaultInformationService.VaultInformation,
            VaultInformationService.VaultInformation._id).then(function (response) {
                $scope.IsEdit = false;
                $scope._new = "";
                $rootScope.$broadcast('socialInsuranceCard');
