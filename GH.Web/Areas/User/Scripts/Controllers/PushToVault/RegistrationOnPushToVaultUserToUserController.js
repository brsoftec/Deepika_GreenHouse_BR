...
myApp.controller('RegistrationOnPushToVaultUserToUserController',
    ['$scope', '$rootScope', '$uibModalInstance', '$uibModal', '$http', 'UserManagementService', 'SweetAlert', 'AuthorizationService', 'alertService', 'NotificationService', 'registerPopup', 'SmSAuthencationService', 'BusinessAccountService', 'NetworkService', 'interactionFormService',
        function ($scope, $rootScope, $uibModalInstance, $uibModal, $http, userManager, sweetAlert, authService, alertService, notificationService, registerPopup, SmSAuthencationService, _baService, networkService, interactionFormService) {
            $scope.messageBox = "";
            $scope.alerts = [];
            ...
            $scope.GetAllFriends = function () {
                networkService.GetFriends().then(function (result) {
            ...
            $scope.RegisterCampaign = function () {
                ...
                if ($scope.memberBusSelected != null && $scope.memberBusSelected != undefined)
                    data.usernotifycationid = $scope.memberBusSelected.Id;
                data.usernotifycationemail = $scope.usernotifycationemail;
                data.pushtype = $scope.pushtype;
                var isvalid = true;
                ...
                if (!isvalid)
                    return;
                $http.post('/api/CampaignService/RegisterCampaignForBus', data)
