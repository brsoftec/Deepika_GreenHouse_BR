...
myApp.controller('RegistrationHandshakeOnNewFeedController',
['$scope', '$rootScope', '$uibModalInstance', '$uibModal', '$http', 'UserManagementService', 'SweetAlert', 'AuthorizationService', 'alertService', 'notificationService', 'registerPopup', 'SmSAuthencationService','fileUpload', 'interactionFormService', 'formService',

function ($scope, $rootScope, $uibModalInstance, $uibModal, $http, userManager, sweetAlert, authService, alertService, notificationService, registerPopup, SmSAuthencationService,fileUpload, interactionFormService, formService) {
    $scope.messageBox = "";
    $scope.alerts = [];
    ...
    $scope.posthndshakecomment = registerPopup.posthndshakecomment
    $scope.Iscustomrole = false;
    $scope.Mode = "Read";
    $scope.IsReadOnly = false;
    $scope.Isregis = true;
    $scope.statusjoin = registerPopup.statusjoin;
    ...
    $scope.groupField = interactionFormService.sortField($scope.dataregisterform);
   ...
   $scope.validateForm = function() {
       var allValid = true;
        angular.forEach($scope.dataregisterform, function(field) {
            var valid = formService.validateField(field);
            if (field.optional && !field.selected)
                valid = true;
            // console.log(field.label, field.type, valid);
            allValid = allValid && valid;
        });
        return allValid;
   };

    $scope.Register = function (team) {
        if (!$scope.validateForm()) {
            alertService.renderErrorMessage("Please fill in all values");
