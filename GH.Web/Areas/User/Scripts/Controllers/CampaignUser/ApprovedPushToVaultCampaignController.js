...
myApp.getController('ApprovedPushToVaultCampaignController',
['$scope', '$rootScope', '$http', 'UserManagementService', 'SweetAlert', 'AuthorizationService', 'alertService', 'InformationVaultService', 'NotificationService', 'CountryCityService', 'fileUpload', 'dateFilter', 'UserNormalCampaignService', 'CommonService', '$uibModal',
function ($scope, $rootScope, $http, userManager, sweetAlert, authService, alertService, informationVaultService, notificationService, countryCityService, fileUpload, dateFilter, CampaignService, CommonService, $uibModal) {
    //var vm = this;
    ...
    $scope.InsertCampaignAdvertising = function (campaignAdvertising) {
        ...
        window.location.href = "/CampaignUser/AdvertisingCampaignPreview";
    }
    ...
    $scope.SaveAsDraff = function () {
        ...
        if (fullcampign._id != "" && $scope.action != "clone") {
            ...
            CampaignService.SaveCampaign(fullcampign).then(function (response) {
                ...
                window.location.href = "/CampaignUser/ManagerCampaign";
            }, function (errors) {
    ...
    $scope.NextToPreview = function () {
     ...
     $scope.showhidebutton = function () {
        ...
        if (fullcampign._id != "" &&  $scope.action!="clone" ) {
            ...
            $scope.isshowbackcampaign = false;
            $scope.isshowtemplate = true;
            $scope.pageTitle = "NEW CAMPAIGN - PUSH TO VAULT";
        }
    }
    ...
    $scope.successFunction3 = function () {
        CampaignService.CampaignAdvertising = null;
        $location.path("CampaignUser/ManagerCampaign");
    }
