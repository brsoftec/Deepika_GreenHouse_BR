...
(function (root, factory) {
     ...
     */
    var lastParentSelector = '';
    wdtEmojiBundle.init = function (selector, parentSelector) {

        if (parentSelector === lastParentSelector) return;
        lastParentSelector = parentSelector;
        var self = this;
        ...
        self.popup = document.querySelector(parentSelector + ' .wdt-emoji-popup');
    ...
    wdtEmojiBundle.openPicker = function (ev) {
        ...
        wdtEmojiBundle.input = document.querySelector(wdtEmojiBundle.selector);
    ...
    wdtEmojiBundle.bindEvents = function () {
        ...
        function onEmojiClick(event) {
            ...
            /*     var html = $(self.render(this.dataset.wdtEmojiShortnames));
     var span = html.find('.emoji-inner');
     span.unwrap();
     span.removeClass('emoji-inner').addClass('msg-emoji');
        wdtEmojiBundle.input.appendChild(span.get(0));*/

            var code = parseInt(self.toCode(this.dataset.wdtEmojiShortname), 16);
            if (code) {
                var selection = getSelection(wdtEmojiBundle.input);
                /// replaceText(wdtEmojiBundle.input, selection, ':' + this.dataset.wdtEmojiShortname + ':');
                //replaceText(wdtEmojiBundle.input, selection, String.fromCodePoint(code));
                wdtEmojiBundle.input.appendChild(document.createTextNode(String.fromCodePoint(code)));
                /*            var img = document.createElement('img');
            img.src = '/Content/img/emojis/s/' + code + '.png';
            img.className = 'msg-emoji';
            wdtEmojiBundle.input.appendChild(img);*/
            }

            wdtEmojiBundle.input.focus();

            if (typeof window.getSelection !== "undefined"
                && typeof document.createRange !== "undefined") {
                var range = document.createRange();
                range.selectNodeContents(wdtEmojiBundle.input);
                ...
                textRange.moveToElementText(wdtEmojiBundle.input);
        ...
        live('click', '.wdt-emoji-list a.wdt-emoji', onEmojiClick);
        ...
        live('mouseover', '.wdt-emoji-list a.wdt-emoji', function (event) {
            ...
            wdtEmojiBundle.previewTimer = setTimeout(function () {
                ...
                wdtEmojiBundle.previewName.innerHTML = emo.dataset.wdtEmojiShortnames;
                wdtEmojiBundle.previewAliases.innerHTML = emo.dataset.wdtEmojiName.toLowerCase();
    ...
    wdtEmojiBundle.toHtml = function (text) {
        var colons = this.emoji.map.colons;
        for (var colon in colons) {
            var rgx = new RegExp(':' + colon + ':');
            text = text.replace(rgx, '<img class="emoji" src="/Content/img/emojis/s/' + colons[colon] + '.png"/>');
        }
        return text;
    };
    wdtEmojiBundle.toCode = function (colon) {
        var colons = this.emoji.map.colons;
        if (colons.hasOwnProperty(colon)) {
            return colons[colon];
        }
        return '';
    };
    ...
    var live = function (eventType, elementQuerySelector, cb) {
        ...
        if (liveSelectors.hasOwnProperty(eventType) && liveSelectors[eventType].hasOwnProperty(elementQuerySelector))
            return;
        // var qs = document.querySelectorAll(elementQuerySelector);

        liveSelectors[eventType] = liveSelectors[eventType] || {};
        liveSelectors[eventType][elementQuerySelector] = true;

        document.addEventListener(eventType, onDoc);
    };
